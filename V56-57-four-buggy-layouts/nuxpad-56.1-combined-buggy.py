...
#
# nuxpad.py
# Version: 57.0 (Unified 4-Way Layout Controller)
# ==============================================================================
...
class Nuxpad:
    def __init__(self, root):
        ...
        # Window Setup
        self.root.title("Untitled - Nuxpad v57.0")
        ...
        self.search_frame = tk.Frame(self.root, bd=0, relief="flat")
        ...
        # Build Layout Based on Preferences
        self.setup_layout()
    ...
    def load_preferences(self):
        # Defaults: layout_mode can be 0: In-line Top, 1: In-line Down, 2: Split Top, 3: Split Down
        ...
        try:
            if os.path.exists(self.config_file):
                with open(self.config_file, "r", encoding="utf-8") as f:
                    ...
                    self.layout_mode = data.get("layout_mode", 0)
    ...
    def setup_layout(self):
        # Clear existing packed frames to rebuild layout cleanly
        ...
        # Determine structural orientation and arrow indicators based on layout_mode
        # 0: In-line Top, 1: In-line Down, 2: Split Top, 3: Split Down
        if self.layout_mode == 0:
            # In-line Top: Toolbar at top (search will be placed on the right inside toolbar)
            self.toolbar.pack(side="top", fill="x", padx=5, pady=5)
            self.editor_frame.pack(side="top", fill="both", expand=True, padx=5, pady=5)
            arrow_dir, arrow_char = "below", "▾"
            inline_search = True
        elif self.layout_mode == 1:
            # In-line Down: Toolbar at bottom (search will be placed on the right inside toolbar)
            self.editor_frame.pack(side="top", fill="both", expand=True, padx=5, pady=5)
            self.toolbar.pack(side="bottom", fill="x", padx=5, pady=5)
            arrow_dir, arrow_char = "above", "▴"
            inline_search = True
        elif self.layout_mode == 2:
            # Split Top: Toolbar Top, Search Down
            self.toolbar.pack(side="top", fill="x", padx=5, pady=5)
            self.editor_frame.pack(side="top", fill="both", expand=True, padx=5, pady=5)
            self.search_frame.pack(side="bottom", fill="x", padx=5, pady=(0, 5))
            arrow_dir, arrow_char = "below", "▾"
            inline_search = False
        else:
            # Split Down: Search Top, Toolbar Down
            self.search_frame.pack(side="top", fill="x", padx=5, pady=(5, 0))
            self.editor_frame.pack(side="top", fill="both", expand=True, padx=5, pady=5)
            self.toolbar.pack(side="bottom", fill="x", padx=5, pady=5)
            arrow_dir, arrow_char = "above", "▴"
            inline_search = False

        self.create_toolbar(arrow_dir, arrow_char, inline_search)
    ...
    def create_toolbar(self, arrow_dir, arrow_char, inline_search):
        self.buttons = []
        self.menus = []
        self.separators = []
        self.search_widgets = []
        ...
        def create_menu_style_button(parent, text):
            mb = tk.Menubutton(parent, text=text, relief="flat", bd=0, padx=6, pady=2, cursor="hand2", direction=arrow_dir)
        ...
        # --- File Menu ---
        self.file_mb, file_menu = create_menu_style_button(self.left_toolbar, f"File {arrow_char}")
        ...
        # --- Edit Menu ---
        self.edit_mb, edit_menu = create_menu_style_button(self.left_toolbar, f"Edit {arrow_char}")
        ...
        # --- Toggles Menu ---
        self.toggles_mb, toggles_menu = create_menu_style_button(self.left_toolbar, f"Toggles {arrow_char}")
        ...
        # Submenu or radio-like behavior for cycling through the 4 layouts
        layout_menu = tk.Menu(toggles_menu, tearoff=0)
        toggles_menu.add_cascade(label="Layout Mode", menu=layout_menu)
        
        layout_menu.add_command(label="1. In-line Top", command=lambda: self.set_layout_mode(0))
        layout_menu.add_command(label="2. In-line Down", command=lambda: self.set_layout_mode(1))
        layout_menu.add_command(label="3. Split Toolbar Top / Search Down", command=lambda: self.set_layout_mode(2))
        layout_menu.add_command(label="4. Split Toolbar Down / Search Top", command=lambda: self.set_layout_mode(3))
        ...
        # If inline layout, create search box inside right toolbar
        if inline_search:
    ...
    def create_search_bar(self):
        self.search_container = tk.Frame(self.search_frame, relief="solid", bd=1)
    ...
    def open_font_dialog(self):
        ...
        tk.Button(btn_frame, text="Apply", command=apply_font_changes, bg=widget_bg, fg=widget_fg, relief="solid", bd=1, padx=10, pady=2).pack(side="right")
        tk.Button(btn_frame, text="Cancel", command=font_win.destroy, bg=widget_bg, fg=widget_fg, relief="solid", bd=1, padx=10, pady=2).pack(side="right", padx=10)
    ...
    def text_cut(self):
        try: self.text_area.event_generate("<<Cut>>")
        except tk.TclError: pass

    def text_copy(self):
        try: self.text_area.event_generate("<<Copy>>")
        except tk.TclError: pass

    def text_paste(self):
        try: self.text_area.event_generate("<<Paste>>")
        except tk.TclError: pass
    ...
    def update_line_numbers(self, event=None):
        ...
        num_fg = "#666666" if self.is_dark_mode else "#999999"
        base_bg = "#1e1e1e" if self.is_dark_mode else "#ffffff"
    ...
    def apply_theme(self):
        if self.is_dark_mode:
            ...
            btn_bg, btn_fg, btn_active_bg = "#1e1e1e", "#d4d4d4", "#333333"
            menu_bg, menu_fg, menu_active_bg, menu_select_color = "#252526", "#d4d4d4", "#333333", "#007acc"
            select_bg, sep_color = "#264f78", "#444444"
            entry_bg, entry_fg, entry_border = "#2d2d2d", "#d4d4d4", "#555555"
        else:
            bg_color, fg_color, insert_color = "#ffffff", "#000000", "#000000"
            btn_bg, btn_fg, btn_active_bg = "#ffffff", "#000000", "#e8e8e8"
            menu_bg, menu_fg, menu_active_bg, menu_select_color = "#ffffff", "#000000", "#e8e8e8", "#000000"
            select_bg, sep_color = "#accced", "#cccccc"
            entry_bg, entry_fg, entry_border = "#ffffff", "#000000", "#7f7f7f"

        self.root.config(bg=bg_color)
        self.toolbar.config(bg=bg_color)
        self.search_frame.config(bg=bg_color)
        ...
        self.text_area.config(bg=bg_color, fg=fg_color, insertbackground=insert_color, selectbackground=select_bg)

        for btn in self.buttons:
            btn.config(bg=btn_bg, fg=btn_fg, activebackground=btn_active_bg, activeforeground=fg_color)

        for menu in self.menus:
            menu.config(bg=menu_bg, fg=menu_fg, activebackground=menu_active_bg, activeforeground=menu_fg, selectcolor=menu_select_color)
        ...
        if hasattr(self, "inline_search_btn"):

...
class Nuxpad:
    def __init__(self, root):
        ...
        # Toolbar Frame (Container for icon buttons) - Placed at the BOTTOM, bezel-less
        ...
        self.text_area = tk.Text(self.editor_frame, wrap="word", undo=True, font=self.normal_font, relief="flat", bd=0, highlightthickness=0)
        ...
        # Build Toolbar Buttons with compact icons
    ...
    def create_toolbar_buttons(self):
        ...
        # Left Frame for icon action buttons
        ...
        def add_btn(parent, icon, command, padx=4):
            btn = tk.Button(parent, text=icon, command=command, relief="flat", bd=0, padx=padx, cursor="hand2")
            btn.pack(side="left", padx=2, pady=2)
            self.buttons.append(btn)
            return btn

        # File & Edit & Formatting Operations represented as clean icons
        add_btn(self.left_toolbar, "📄", self.new_file)       # New
        add_btn(self.left_toolbar, "📂", self.open_file)      # Open
        add_btn(self.left_toolbar, "💾", self.save_file)      # Save
        add_btn(self.left_toolbar, "↩️", self.text_edit_undo)  # Undo
        add_btn(self.left_toolbar, "↪️", self.text_edit_redo)  # Redo
        add_btn(self.left_toolbar, "✂️", self.text_cut)       # Cut
        add_btn(self.left_toolbar, "📋", self.text_copy)      # Copy
        add_btn(self.left_toolbar, "📌", self.text_paste)     # Paste
        add_btn(self.left_toolbar, "🔍", self.find_text)      # Find
        add_btn(self.left_toolbar, "mathbf{B}", lambda: self.toggle_tag("bold")) # Bold placeholder icon/text
        # Let's use clean text glyphs or symbols for text styling icons to stay compact and pretty:
        # B, I, U, List symbols can use styled text indicators safely
        # Let's redefine the format buttons cleanly:
        # Re-populating with explicit icon mappings:
    ...
    def create_toolbar_buttons(self):
        ...
        def add_btn(parent, icon, command, padx=5):
            btn = tk.Button(parent, text=icon, command=command, relief="flat", bd=0, padx=padx, cursor="hand2")
            btn.pack(side="left", padx=2, pady=2)
            self.buttons.append(btn)
            return btn

        # Icon-only compact toolbar layout
        add_btn(self.left_toolbar, "📄", self.new_file)       # New File
        add_btn(self.left_toolbar, "📁", self.open_file)      # Open File
        add_btn(self.left_toolbar, "💾", self.save_file)      # Save File
        add_btn(self.left_toolbar, "↶", self.text_edit_undo)  # Undo
        add_btn(self.left_toolbar, "↷", self.text_edit_redo)  # Redo
        add_btn(self.left_toolbar, "✂", self.text_cut)        # Cut
        add_btn(self.left_toolbar, "📋", self.text_copy)      # Copy
        add_btn(self.left_toolbar, "📋", self.text_paste)     # Paste (using clipboard icon)
        add_btn(self.left_toolbar, "🔍", self.find_text)      # Find
        add_btn(self.left_toolbar, "𝐁", lambda: self.toggle_tag("bold"))      # Bold
        add_btn(self.left_toolbar, "𝐼", lambda: self.toggle_tag("italic"))    # Italic
        add_btn(self.left_toolbar, "̲U", lambda: self.toggle_tag("underline")) # Underline
        add_btn(self.left_toolbar, "≡", self.insert_bullet_point)             # Bullet List

...
#
# nuxpad.py
# Version: 29 (Fixed Arrows Orientation)
# ==============================================================================
...
class Nuxpad:
    ...
    def create_toolbar(self):
        ...
        # --- Left Format Buttons ---
        create_action_menubutton(self.left_toolbar, "Bold", lambda: self.set_exclusive_tag("bold"))
        create_action_menubutton(self.left_toolbar, "Italic", lambda: self.set_exclusive_tag("italic"))
        create_action_menubutton(self.left_toolbar, "Underline", lambda: self.set_exclusive_tag("underline"))
    ...
    def set_exclusive_tag(self, tag_name):
        """Clears bold, italic, and underline tags on the selection, then applies the chosen one uniquely."""
        try:
            if self.text_area.tag_ranges("sel"):
                # Remove all style tags first to prevent overlap conflicts
                for t in ["bold", "italic", "underline"]:
                    self.text_area.tag_remove(t, "sel.first", "sel.last")
                # Apply the requested tag cleanly

...
# ==============================================================================
# Nuxpad
# Copyright (C) 2026 AI Collaborator
#
...
class Nuxpad:
    def __init__(self, root):
        ...
        # Track text modifications and updates for line numbers & character count
        self.text_area.bind('<<Modified>>', self.on_text_modified)
        self.text_area.bind('<KeyRelease>', self.update_line_numbers_and_stats)
    ...
    def create_toolbar(self):
        ...
        # Right Frame for Search, Character Counter, and Separator
        ...
        # Theme label will dynamically reflect mode and icon
        theme_label = "🌙  Dark Mode" if not self.is_dark_mode else "☀️  Light Mode"
        toggles_menu.add_command(label=theme_label, command=self.toggle_theme)
        self.toggles_menu_ref = toggles_menu
        ...
        # --- Right Toolbar Components (Search, Separator, Character Counter) ---
        create_action_menubutton(self.right_toolbar, "Search", self.find_text)

        right_sep = tk.Frame(self.right_toolbar, width=2, bd=0, relief="sunken")
        right_sep.pack(side="left", fill="y", padx=6, pady=4)
        self.separators.append(right_sep)

        # Character Counter Label
        self.char_count_label = tk.Label(self.right_toolbar, text="Chars: 0", padx=6, pady=2, font=self.normal_font)
        self.char_count_label.pack(side="left", padx=2, pady=2)
    ...
    def insert_bullet_point(self):
        ...
        if line_content.strip() == "":
            ...
            self.text_area.insert(current_index, "\n• ")
        self.update_line_numbers_and_stats()

    def update_character_count(self):
        # Get all text content minus the trailing newline character standard to tk.Text
    ...
    def update_line_numbers_and_stats(self, event=None):
    ...
    def apply_theme(self):
        if self.is_dark_mode:
            ...
            select_bg = "#accced"
            sep_color = "#cccccc"
            new_theme_label = "🌙  Dark Mode"

        # Update Toggles menu item text dynamically
        if hasattr(self, 'toggles_menu_ref'):
            self.toggles_menu_ref.entryconfig(2, label=new_theme_label)

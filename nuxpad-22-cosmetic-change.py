...
class Nuxpad:
    ...
    def create_toolbar(self):
        ...
        # --- Formatting Buttons & Search on Right ---
        def add_right_btn(text, command, padx=5):
            btn = tk.Button(self.right_toolbar, text=text, command=command, relief="flat", bd=0, padx=padx, cursor="hand2")
            btn.pack(side="left", padx=2, pady=2)
            self.buttons.append(btn)
            return btn

        add_right_btn("𝐁", lambda: self.toggle_tag("bold"))
        add_right_btn("𝐼", lambda: self.toggle_tag("italic"))
        add_right_btn("̲U", lambda: self.toggle_tag("underline"))
        add_right_btn("• List", self.insert_bullet_point)
        add_right_btn("🔍", self.find_text) # Search button placed last on the right
    ...
    def apply_theme(self):
        if self.is_dark_mode:
            ...
            menu_select_color = "#007acc" # Bright tick mark/indicator color for dark mode checkbuttons
            select_bg = "#264f78"
        else:
            bg_color = "#ffffff"
            fg_color = "#000000"
            ...
            menu_select_color = "#000000" # Clear dark tick mark for light mode checkbuttons
            select_bg = "#accced"

...
class Nuxpad:
    ...
    def create_toolbar(self):
        ...
        # Left Frame for Menus
        self.left_toolbar = tk.Frame(self.toolbar, bd=0, relief="flat")
        self.left_toolbar.pack(side="left", anchor="w")

        # Center Frame for Format Actions (Bold, Italic, Underline, List)
        self.center_toolbar = tk.Frame(self.toolbar, bd=0, relief="flat")
        self.center_toolbar.pack(side="left", expand=True)
        ...
        # --- Menubutton Helper (Used for menus and menu-style buttons) ---
        def create_menu_button(parent, text, command=None):
            if command:
                btn = tk.Button(parent, text=text, command=command, relief="flat", bd=0, padx=6, pady=2, cursor="hand2")
            else:
                btn = tk.Menubutton(parent, text=text, relief="flat", bd=0, padx=6, pady=2, cursor="hand2")
        ...
        # --- File Menu ---
        file_mb = create_menu_button(self.left_toolbar, "File ▾")
        file_menu = file_mb.nametowidget(file_mb.menuname)
        ...
        # --- Edit Menu ---
        edit_mb = create_menu_button(self.left_toolbar, "Edit ▾")
        edit_menu = edit_mb.nametowidget(edit_mb.menuname)
        ...
        # --- Toggles Menu ---
        toggles_mb = create_menu_button(self.left_toolbar, "Toggles ▾")
        toggles_menu = toggles_mb.nametowidget(toggles_mb.menuname)
        ...
        # --- Centered Format Buttons (Menu Style) ---
        create_menu_button(self.center_toolbar, "Bold", lambda: self.toggle_tag("bold"))
        create_menu_button(self.center_toolbar, "Italic", lambda: self.toggle_tag("italic"))
        create_menu_button(self.center_toolbar, "Underline", lambda: self.toggle_tag("underline"))
        create_menu_button(self.center_toolbar, "List", self.insert_bullet_point)

        # --- Right Search Button (Menu Style) ---
        create_menu_button(self.right_toolbar, "Search", self.find_text)
    ...
    def apply_theme(self):
        ...
        self.center_toolbar.config(bg=bg_color)

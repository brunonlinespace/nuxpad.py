...
#
# nuxpad.py
# Version: 49.4 (Toggleable In-line Search Bar - Buggy Function)
# ==============================================================================
...
class Nuxpad:
    def __init__(self, root):
        ...
        # Window Setup
        self.root.title("Untitled - Nuxpad v49.4")
        ...
        # Split Search Bar Frame (Placed at the top of the text area when split configuration is active)
        self.split_search_frame = tk.Frame(self.root, bd=0, relief="flat")
        ...
        # Build Toolbar Components & Search Bar Positioning
        self.create_toolbar()
        self.update_search_bar_layout()
        ...
        try:
            if os.path.exists(self.config_file):
                with open(self.config_file, "r", encoding="utf-8") as f:
                    ...
                    self.inline_toolbar = data.get("inline_toolbar", True)
    ...
    def create_toolbar(self):
        ...
        self.inline_toolbar_var = tk.BooleanVar(value=self.inline_toolbar)
        ...
        toggles_menu.add_checkbutton(label="In-line Toolbar", variable=self.inline_toolbar_var, command=self.toggle_inline_toolbar)
    ...
    def update_search_bar_layout(self):
        # Forget pack/unpack state first to cleanly relocate
        self.search_container.pack_forget()
        self.split_search_frame.pack_forget()

        if self.inline_toolbar:
            # Place back into the right side of the main bottom toolbar
            self.search_container.pack(side="right", padx=2, pady=2)
        else:
            # Place at the top of the text area area (above editor_frame pack hierarchy)
            self.split_search_frame.pack(side="top", fill="x", padx=5, pady=(5, 0))
            self.search_container.pack(side="right", padx=2, pady=2)
    ...
    def toggle_inline_toolbar(self):
        self.inline_toolbar = self.inline_toolbar_var.get()
        
        # Reparent search container based on layout selection
        if self.inline_toolbar:
            self.search_container.master = self.right_toolbar
        else:
            self.search_container.master = self.split_search_frame
    ...
    def apply_theme(self):
        ...
        self.split_search_frame.config(bg=bg_color)

...
#
# nuxpad.py
# Version: 52.2 (Buggy New In-line Search Toggle)
# ==============================================================================
...
class Nuxpad:
    def __init__(self, root):
        ...
        # Window Setup
        self.root.title("Untitled - Nuxpad v52.2")
        ...
        # Top Search Bar Frame - Placed above the editor (conditionally packed)
        self.top_search_frame = tk.Frame(self.root, bd=0, relief="flat")
        if not self.inline_search:
        ...
        editor_pady = 0 if not self.inline_search else 5
        self.editor_frame.pack(side="top", fill="both", expand=True, padx=5, pady=editor_pady)
        ...
        # Build Toolbar & Search Components
        self.create_toolbar()
        ...
        try:
            if os.path.exists(self.config_file):
                with open(self.config_file, "r", encoding="utf-8") as f:
                    ...
                    self.inline_search = data.get("inline_search", True)
    ...
    def create_toolbar(self):
        ...
        # Right Frame for Search Box Container (used when in-line)
        ...
        self.inline_search_var = tk.BooleanVar(value=self.inline_search)
        ...
        toggles_menu.add_checkbutton(label="In-line Search", variable=self.inline_search_var, command=self.toggle_inline_search)
    ...
    def create_search_components(self):
        ...
        # Determine target container based on inline_search preference
        target_parent = self.right_toolbar if self.inline_search else self.top_search_frame

        self.search_container = tk.Frame(target_parent, relief="solid", bd=1)
        if self.inline_search:
            ...
            # Matches nuxpad-48 style: spans across the top above the text area
        ...
        if self.inline_search:
            self.search_entry.config(width=18)
        self.search_entry.pack(side="left", padx=entry_padx, pady=entry_pady, fill=entry_fill, expand=entry_expand)
        self.search_widgets.append(self.search_entry)
        self.search_entry.bind("<Return>", lambda event: self.find_text())

        self.inline_search_btn = tk.Label(self.search_container, text=icon_text, relief="flat", bd=0, padx=icon_padx, pady=0 if self.inline_search else 2, cursor="hand2")
    ...
    def toggle_inline_search(self):
        self.inline_search = self.inline_search_var.get()
        self.save_preferences()
        
        # Re-create and position search components cleanly
        if hasattr(self, 'search_container'):
            self.search_container.destroy()
            
        if self.inline_search:
            self.top_search_frame.pack_forget()
            self.editor_frame.pack_configure(padx=5, pady=5)
        else:
            self.editor_frame.pack_configure(padx=5, pady=0)
            self.top_search_frame.pack(side="top", fill="x", padx=5, pady=(5, 0))

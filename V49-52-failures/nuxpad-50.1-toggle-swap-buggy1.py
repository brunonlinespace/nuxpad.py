...
#
# nuxpad.py
# Version: 50.1 (Buggy Top / Down Mode Toggle for Toolbar and Search Positions)
# ==============================================================================
...
class Nuxpad:
    def __init__(self, root):
        ...
        # Window Setup
        self.root.title("Untitled - Nuxpad v50.1")
        ...
        # Toolbar Frame (Bottom or Top depending on layout preference)
        self.toolbar = tk.Frame(self.root, bd=0, relief="flat")
        
        # Search Bar Frame (Top or Bottom depending on layout preference)
        self.top_search_frame = tk.Frame(self.root, bd=0, relief="flat")

        # Apply initial positions based on loaded preference
    ...
    def apply_layout_positions(self):
        # Clear packing orders first
        self.toolbar.pack_forget()
        self.top_search_frame.pack_forget()

        if self.top_down_mode:
            # Top / Down Mode: Toolbar at top, Search box at bottom
            self.toolbar.pack(side="top", fill="x", padx=5, pady=(5, 0))
            self.top_search_frame.pack(side="bottom", fill="x", padx=5, pady=5)
        else:
            # Default Mode: Search box at top, Toolbar at bottom
        ...
        try:
            if os.path.exists(self.config_file):
                with open(self.config_file, "r", encoding="utf-8") as f:
                    ...
                    self.top_down_mode = data.get("top_down_mode", False)
    ...
    def create_toolbar(self):
        ...
        # Clear existing toolbar children if rebuilding
        ...
        self.topdown_var = tk.BooleanVar(value=self.top_down_mode)
        ...
        toggles_menu.add_checkbutton(label="Top / Down Mode", variable=self.topdown_var, command=self.toggle_top_down_mode)
    ...
    def create_top_search(self):
        ...
        # Clear existing search children if rebuilding
    ...
    def toggle_top_down_mode(self):
        self.top_down_mode = self.topdown_var.get()
        self.apply_layout_positions()
        self.save_preferences()

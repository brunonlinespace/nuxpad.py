...
class Nuxpad:
    ...
    def create_toolbar(self):
        ...
        def create_action_menubutton(parent, text, command, custom_font=None):
            mb = tk.Menubutton(parent, text=text, relief="flat", bd=0, padx=6, pady=2, cursor="hand2")
            if custom_font:
                mb.config(font=custom_font)
            # Use 'above' direction to prevent menus/actions from collapsing incorrectly when hitting screen edges
            mb.config(direction="above")
        ...
        file_menu.config(direction="above")
        ...
        edit_menu.config(direction="above")
        ...
        toggles_menu.config(direction="above")
        ...
        create_action_menubutton(self.left_toolbar, "Bold", lambda: self.toggle_tag("bold"), custom_font=self.bold_btn_font)
        create_action_menubutton(self.left_toolbar, "Italic", lambda: self.toggle_tag("italic"), custom_font=self.italic_btn_font)
        create_action_menubutton(self.left_toolbar, "Underline", lambda: self.toggle_tag("underline"), custom_font=self.underline_btn_font)
    ...
    def check_save_changes(self):
        if not self.content_saved:
            # Custom stylized dialog box matching theme consistency
    ...
    def custom_yesnocancel(self, title, message):
        """Custom themed modal dialog box to ensure consistent UI styling."""
    ...
    def find_text(self):
        ...
        # Custom themed prompt dialog matching styling guidelines

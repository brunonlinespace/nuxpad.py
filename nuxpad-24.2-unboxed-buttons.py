...
class Nuxpad:
    ...
    def create_toolbar(self):
        ...
        # --- Centered Format Buttons (Menu Style using Menubutton without arrow feel) ---
        def create_action_menubutton(parent, text, command):
            mb = tk.Menubutton(parent, text=text, relief="flat", bd=0, padx=6, pady=2, cursor="hand2")
            # Override behavior to act as a direct button click instead of opening an empty menu
            mb.config(direction="below")
            mb.bind("<Button-1>", lambda e: command())
            mb.pack(side="left", padx=2, pady=2)
            self.buttons.append(mb)
            return mb

        create_action_menubutton(self.center_toolbar, "Bold", lambda: self.toggle_tag("bold"))
        create_action_menubutton(self.center_toolbar, "Italic", lambda: self.toggle_tag("italic"))
        create_action_menubutton(self.center_toolbar, "Underline", lambda: self.toggle_tag("underline"))
        create_action_menubutton(self.center_toolbar, "List", self.insert_bullet_point)
